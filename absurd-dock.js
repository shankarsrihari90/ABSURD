(function () {
  ...
  var experiences = [['tool-button','USELESS','BUTTON'],['tool-ask','ASK','ANYTHING'],['tool-mood','MOOD','MACHINE'],['products','ABSURD','PRODUCTS'],['tool-dad','DAD','JOKES'],['tool-veg','VEG','DATING']];
  ...
  function closeLauncher() { var launcher = document.getElementById('dock-launcher'), toggle = document.querySelector('.dock-expand'); if (launcher) launcher.classList.remove('open'); if (toggle) { toggle.setAttribute('aria-expanded', 'false'); toggle.textContent = '6 THINGS ↑'; } }
  function go(id) { closeLauncher(); if (id === 'products') { record(id); window.openAbsurdProducts(); return; } record(id); window.switchView(id); }
  function random() { var options = experiences.concat([['tool-game'],['mia']]), choice = options[Math.floor(Math.random() * options.length)], data = read(); data.random++; write(data); if (choice[0] === 'mia') { record('mia'); closeLauncher(); window.__MIA3__ && window.__MIA3__.open(); return; } go(choice[0]); }
  ...
  function mount() {
    ...
    dock.innerHTML = '<div class="dock-frame"><button class="dock-expand" type="button" data-action="features" aria-expanded="false">6 THINGS ↑</button><div class="dock-launcher" id="dock-launcher"></div><div class="dock-main"><button type="button" data-action="home"><span class="dock-icon">⌂</span>Home</button><button type="button" data-action="games"><span class="dock-icon">◉</span>Games</button><button type="button" class="dock-random" data-action="random"><span class="dock-icon">?</span>Random</button><button type="button" class="dock-profile" data-action="profile"><span class="dock-icon">▣</span>Profile</button></div></div>';
    ...
    dock.addEventListener('click', function (event) {
      ...
      if (action) { var name = action.getAttribute('data-action'); if (name === 'features') { var shown = launcher.classList.toggle('open'); action.setAttribute('aria-expanded', String(shown)); action.textContent = shown ? '6 THINGS ↓' : '6 THINGS ↑'; } if (name === 'home') { closeLauncher(); window.switchView('view-hub'); } if (name === 'games') { closeLauncher(); go('tool-game'); } if (name === 'random') random(); if (name === 'profile') { closeLauncher(); renderProfile(); } }
    ...
    document.getElementById('enter-absurd').addEventListener('click', function () { var data = read(); data.visits++; write(data); setTimeout(sync, 0); }); sync();
  }
